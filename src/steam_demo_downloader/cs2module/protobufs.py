...
def find_project_root(
  start_path: Path,
) -> Path:  # required because im too lazy to readjust paths every major change
  root_markers = {
    "src",
    ".git",
  }  # with the way we plan our directories, .env cannot be an anchor point
  current = start_path
...
def build(force=False):
  if force or needs_rebuild():
    ...
    if TEMP_BUILD_DIR.exists():
      shutil.rmtree(TEMP_BUILD_DIR)  # create temp dir
