...
class CS2Client(GameCoordinator):
  ...
  def set_target_match(self, sharecode):
    from csgo.sharecode import (
      decode,
    )  # add decode in here as its the only function that requires it
  ...
  def send_hello(self):
    ...
    self.send(
      header, hello.SerializeToString()
    )  # string conversion before send because steam wants it that way
