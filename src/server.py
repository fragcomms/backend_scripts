...
#DATABASE
async def insert_into_db(record: dict):
    if not db_pool:
...
async def handle_subprocess_event(event: dict, task_name: str):
    ...
    if event_type == "download_complete":
        ...
        await insert_into_db(db_record)
        
    elif event_type == "transcribe_complete":
        logger.info(f"Transcription ready at: {payload.get('txt_path')}")
...
async def launch_subprocess(cmd: list, task_name: str):
    ...
    if working_dir == "": working_dir = None
...
async def lifespan(app: FastAPI):
    ...
    downloader_process = await launch_subprocess([sys.executable, DOWNLOADER_SCRIPT], "Downloader")
...
async def trigger_parse(req: ParseRequest, background_tasks: BackgroundTasks):
    ...
    TASK_CONTEXT[task_name] = {"match_code": req.match_code, "fetch_time": req.fetch_time}
