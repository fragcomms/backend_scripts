...
MAP_GLOSSARY = {
  "Generic": ["CT-Spawn", "T-Spawn", "CT", "T", "Spawn", "A-Site", "B-Site", "A", "B"],
  "Nuke": [
    "Silo",
    "Marshmallow",
    "Squeaky",
...
def main(input_mka, map_name="Nuke"):
  ...
  print(
    f"Found {track_count} audio tracks: {list(track_info.values())}", file=sys.stdout
  )
  ...
  for track_id, segments in all_tracks_data.items():
    for seg in segments:
      ...
      # 2. THEN SEND TO LLM
      flat_prompts.append(
        generate_llm_prompt(current_text, map_name, hotwords, GENERAL_CS2_TERMS)
      )
  ...
  if flat_prompts:
    ...
    for output, seg_ref in zip(outputs, flat_refs):
      ...
      clean_text = re.sub(
        r"^(Output|Fixed|Corrected):\s*", "", clean_text, flags=re.IGNORECASE
      )
...
if __name__ == "__main__":
  if len(sys.argv) < 2:
    print(
      "Usage: python transcriber.py <path_to_audio.mka> [map_name]", file=sys.stderr
    )
    sys.exit(1)
