    ...
    def correct_transcriptions(self, all_tracks_data, map_name, hotwords, general_terms):
        ...
        for track_id, segments in all_tracks_data.items():
            for seg in segments:
                ...
                flat_prompts.append(self.generate_prompt(current_text, map_name, hotwords, general_terms))
        ...
        if flat_prompts:
            ...
            for output, seg_ref in zip(outputs, flat_refs):
                ...
                clean_text = re.sub(r'^(Output|Fixed|Corrected|Current):\s*', '', clean_text, flags=re.IGNORECASE)
